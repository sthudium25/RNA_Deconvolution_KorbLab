...
analysisName = "Marchetto_Deconvolution_DESeqNormCounts"
...
# Read in count data
countData = read.csv("GeneCountFiles/Marchetto_Deconvolution_DESeqNormCounts.csv", row.names = 1) 
...
fPC = lm(eigenGene ~ Dx + principalComps$x[,1:15], data=GEO_data_pd_filtered)

mod = model.matrix(~Dx + principalComps$x[,1:15], 
...
pdf(sprintf("figures/%s_korbASDSigDown_deconvolution2.pdf", analysisName),h=6,w=5)
...
title(main = sprintf("%s\nacct for Dx, 15 PCs", analysisName))
dev.off()
